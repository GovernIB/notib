	...
	public static void comprovarPermisosProcedimentsUsuariActual(
		...
		if (RolHelper.isUsuariActualUsuari(request)) {
			...
			if (entitatActual != null && usuariActual != null && RolHelper.isUsuariActualUsuari(request)) {
				...
				List<ProcSerSimpleDto> procedimentsDisponiblesPerSir = procedimentService.findProcedimentServeisWithPermisMenu(entitatActual.getId(), usuariActual.getCodi(), PermisEnum.COMUNIACIO_SIR);
				request.setAttribute("permisNotificacioComunicacioMenu", !procedimentsDisponibles.isEmpty());
				request.setAttribute("permisComunicacioSirMenu", !procedimentsDisponiblesPerSir.isEmpty());
			}
		}
	}
