...
import es.caib.notib.war.command.ProcSerPermisCommand;
...
public class ProcedimentPermisController extends BaseUserController{
	...
	public DatatablesResponse datatable(HttpServletRequest request, @PathVariable Long procedimentId, Model model) {
		...
		List<PermisDto> permisos = procedimentService.permisFind(entitatActual.getId(), isAdministrador(request), procedimentId, null, organCodi, null, paginacioParams);
	...
	private String getPermis(HttpServletRequest request, Long procedimentId, Long permisId, Model model, TipusPermis tipus, String organ, PaginacioParamsDto paginacioParams) {
		...
		if (permisId != null) {
			String organCodi = organGestorActual != null ? organGestorActual.getCodi() : null;
			List<PermisDto> permisos = procedimentService.permisFind(entitatActual.getId(), isAdministrador(request), procedimentId, organ, organCodi, tipus, paginacioParams);
