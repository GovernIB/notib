...
public class ExportacioExcelGrup extends ExportacioExcel {
	...
	public ExportacioExcelGrup() {
		super();
	}

	public byte[] convertDadesGrup(
			Map<GrupDto, List<HistoricAggregationGrupDto>> dades) throws IOException {

		for (GrupDto grup : dades.keySet()) {
			HSSFSheet sheet = wb.createSheet(grup.getCodi() + " - " + grup.getNom());
