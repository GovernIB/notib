...
//            model.addAttribute("oficinesEntitat", organService.getOficinesSIR(entitat.getId(), entitat.getDir3Codi(), true));
            Arbre<OrganGestorDto> arbre = organService.generarArbreOrgans(entitat.getDir3Codi(), filtres.asDto());
...
//            OrganDetall detall = OrganDetall.toOrganDetall(organ);
    ...
    @ResponseBody
    public OrganGestorDto getOrgan(HttpServletRequest request, @PathVariable("codiSia") String codiSia, Model model) {

        try {
            EntitatDto entitat = entitatService.findById(controller.getEntitatActualComprovantPermisos(request).getId());
            OrganGestorDto o = organService.findByCodi(entitat.getId(), codiSia);
            o = o == null ? organService.getOrganNou(codiSia) : o;
//            OrganDetall organ = OrganDetall.builder().id(o.getId()).nom(o.getNom()).codi(o.getCodi())
//                    .oficinaEntitat(entitat.getOficina())
...
//            MissatgesHelper.error(request, ex.getMessage());
    ...
    private void omplirModel(Model model, EntitatDto entitat, OrganGestorDto organ) {
        ...
        command.setId(organ != null ? organ.getId() : null);
        command.setCodi(organ != null ? organ.getCodi() : null);
        command.setId(organ != null ? organ.getId() : null);
        model.addAttribute("organGestorCommand", command);
        model.addAttribute("entitat", entitat);
        model.addAttribute("setLlibre", !entitat.isLlibreEntitat());
        if (organ != null) {
            model.addAttribute("llibres", organService.getLlibreOrganisme(entitat.getId(), organ.getCodi()));
        }
        model.addAttribute("setOficina", !entitat.isOficinaEntitat());
        List<OficinaDto> oficines = organService.getOficinesSIR(entitat.getId(), entitat.getDir3Codi(),true);
