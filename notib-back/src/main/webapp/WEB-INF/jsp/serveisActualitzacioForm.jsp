...
<html>
<head>
	<title><spring:message code="servei.actualitzacio.auto"/></title>
	...
	<script>
		var itervalProgres;
		var writtenLines = 0;
		var title="<spring:message code="servei.actualitzacio.auto"/>";
		var content="<spring:message code="servei.actualitzacio.cancelarActu"/>";
		var acceptar="<spring:message code="servei.actualitzacio.acceptar"/>";
		var cancelar="<spring:message code="servei.actualitzacio.cancelar"/>";
...
// 			itervalProgres = setInterval(getProgres, 250);
		...
		function getProgres() {
			...
			$.ajax({
				type: 'GET',
				url: "<c:url value='/servei/update/auto/progres'/>",
...
<body>
	<c:if test="${not isUpdatingProcediments}">
		...
		<c:set var="formAction"><not:modalUrl value="/servei/update/auto"/></c:set>
	...
	<c:if test="${isUpdatingProcediments}">
		<div class="confirmacio">
			<h4><spring:message code="servei.actualitzacio.procesActiu"/></h4>
		</div>
	</c:if>
</body>
</html>
