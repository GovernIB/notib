            ...
            if (!found) {
                ...
                apiCreate({ data }).
                    then(() => {
                        ...
                        temporalMessageShow(t('page.entitats.form.tipusDocuments.enable.error'), error.message, 'error');
                    });
            }
        } else {
            if (found) {
                apiDelete(found.id).
                    then(() => {
                        ...
                        temporalMessageShow(t('page.entitats.form.tipusDocuments.disable.error'), error.message, 'error');
                    });
            }
        }
    }
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
                ...
                <TableBody>
                    {Object.entries(tipusDocumentOptions).map(([key, value]) => {
                        const checked = entitatTipusDocumentRows.find(r => r.tipusDocument === key) != null;
                        return <TableRow
                            key={key}
