package es.caib.notib.back.resourcecontroller;

import es.caib.notib.back.base.controller.BaseMutableResourceController;
import es.caib.notib.logic.intf.base.config.BaseConfig;
import es.caib.notib.logic.intf.model.AccioMassivaElementResource;
import es.caib.notib.logic.intf.model.AccioMassivaResource;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

/**
 * Servei REST de gestió d'elements de les accions massives.
 *
 * @author Límit Tecnologies
 */
@RestController
@RequestMapping(BaseConfig.API_PATH + "/accions/massives/elements")
public class AccioMassivaElementResourceController extends BaseMutableResourceController<AccioMassivaElementResource, Long> {

}
