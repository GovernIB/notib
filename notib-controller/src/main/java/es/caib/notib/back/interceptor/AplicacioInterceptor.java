...
public class AplicacioInterceptor implements AsyncHandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (manifestAtributsMap == null) {
			...
			if (is != null) {
				...
				var locale = new Locale(RequestContextUtils.getLocale(request).getLanguage(), RequestContextUtils.getLocale(request).getCountry());
				manifestAtributsMap.put("Build-Timestamp", formatBuildTimestamp(manifestAtributsMap.get("Build-Timestamp").toString(), locale));
			}
		}
		request.setAttribute(REQUEST_ATTRIBUTE_MANIFEST_ATRIBUTES, manifestAtributsMap);
		request.setAttribute("requestLocale", RequestContextUtils.getLocale(request).getLanguage());
		return true;
	}
	...
	private String formatBuildTimestamp(String timestamp, Locale locale) throws ParseException {
		...
		var dateFormat = new SimpleDateFormat("YYYY.MM.dd HH:mm", locale);
