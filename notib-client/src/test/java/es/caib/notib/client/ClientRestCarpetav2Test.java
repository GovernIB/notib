...
//		System.setProperty("javax.net.ssl.trustStorePassword", "tecnologies");
	...
	public void testPaginacio() throws Exception {
		...
		RespostaConsultaV2 resposta = client.notificacionsByTitular(DNI_TITULAR, sdf.parse(DATA_INICI), sdf.parse(DATA_FI), VISIBLE, IdiomaEnumDto.CA, 0, mida);
		...
		for (int foo=0;foo<nPagines;foo++) {
			resposta = client.notificacionsByTitular(DNI_TITULAR, sdf.parse(DATA_INICI), sdf.parse(DATA_FI), VISIBLE, IdiomaEnumDto.CA, foo, mida);
			assertNotNull(resposta);
