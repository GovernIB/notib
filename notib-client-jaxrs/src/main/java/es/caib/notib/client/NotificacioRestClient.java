...
/**
 * Client REST v1 per al servei de notificacions de NOTIB (JAX-RS).
 * 
...
public class NotificacioRestClient extends NotificacioBaseRestClient {
	...
	public RespostaAlta alta(NotificacioV2 notificacio) {
		try {
			...
			log.error("Error en alta de notificació", ex);
			RespostaAlta respostaAlta = new RespostaAlta();
			respostaAlta.setError(true);
			respostaAlta.setErrorDescripcio("[CLIENT] Error en alta: "  + ex.getMessage());
			return respostaAlta;
		}
	}
