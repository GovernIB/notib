...
/**
 * Acció per a marcar un procediment perquè s'actualitzi de forma manual.
 */
@Slf4j
@AllArgsConstructor
public class ProcedimentSyncManualActionExecutor implements BaseMutableResourceService.ActionExecutor<ProcedimentResourceEntity, Serializable, Boolean> {
	...
	public Boolean exec(String code, ProcedimentResourceEntity entity, Serializable params) throws ActionExecutionException {
		...
		try {
			procedimentService.updateManual(entity.getId(), true);
			return true;
		} catch (Exception ex) {
			var msg = "Error al marcar el procediment " + entity.getId() + " per a sincronització manual: ";
			log.error(msg, ex);
