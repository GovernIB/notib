...
public class NotificacioMassivaCsvNotificacioReportGenerator implements BaseReadonlyResourceService.ReportGenerator<NotificacioMassivaResourceEntity, Serializable, NotificacioMassivaResource> {
	...
	public List<NotificacioMassivaResource> generateData(String code, NotificacioMassivaResourceEntity entity, Serializable params) throws ReportGenerationException {
		...
		resource.setCsvFilename(entity.getCsvFilename());
	...
	public DownloadableFile generateFile(String code, List<?> data, ReportFileType fileType, OutputStream out) {
		...
		try {
			var file = notificacioMassivaService.getCSVFile(massiva.getEntitat().getId(), massiva.getId());
			if (file.getContingut() == null || file.getContingut().length == 0) {
				log.error("[NotificacioMassivaCsvNotificacioReportGenerator] Error generant CSV de la notificacio massiva " + massiva.getId());
				return DownloadableFile.builder().name(massiva.getCsvFilename()).content(new byte[]{}).contentType(mediaType).build();
			}
			return DownloadableFile.builder().name(massiva.getCsvFilename()).content(file.getContingut()).contentType(mediaType).build();
		} catch (Exception ex) {
			log.error("[NotificacioMassivaCsvNotificacioReportGenerator] Error inesperat generant contingut pel CSV de la notificacio massiva" + massiva.getId(), ex);
			return DownloadableFile.builder().name("error_" + massiva.getCsvFilename()).content(new byte[]{}).contentType(mediaType).build();
		}
	}
