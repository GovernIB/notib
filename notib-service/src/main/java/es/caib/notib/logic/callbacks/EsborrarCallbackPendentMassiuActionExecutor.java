...
@Slf4j
@AllArgsConstructor
public class EsborrarCallbackPendentMassiuActionExecutor implements BaseMutableResourceService.ActionExecutor<CallbackResourceEntity, AccioMassivaParams, CallbackResposta> {
	...
	public CallbackResposta exec(String code, CallbackResourceEntity entity, AccioMassivaParams params) throws ActionExecutionException {
		...
		try {
 			return callbackService.esborrarCallback(new HashSet<>(params.getIds()));
		} catch (Exception ex) {
			var msg = "Error inesperat esborrant els callbacks pendents ";
			log.error("[EsborrarCallbackPendentMassiuActionExecutor] " + msg + "amb id " + entity.getId() + " - " + ex.getMessage());
