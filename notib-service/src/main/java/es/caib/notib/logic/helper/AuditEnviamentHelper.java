...
public class AuditEnviamentHelper {
	...
	public NotificacioEnviamentEntity desaEnviament(EntitatEntity entitat, NotificacioEntity notificacioEntity, Enviament enviament, ServeiTipusEnumDto serveiTipus,
													PersonaEntity titular, List<PersonaEntity> destinataris) {
		...
		return notificacioEnviamentRepository.saveAndFlush(env);
	}
	
	@Audita(entityType = TipusEntitat.ENVIAMENT, operationType = TipusOperacio.CREATE)
	public NotificacioEnviamentEntity desaEnviamentAmbReferencia(EntitatEntity entitat, NotificacioEntity notificacioGuardada, Enviament enviament,
																 ServeiTipusEnumDto serveiTipus, PersonaEntity titular, List<PersonaEntity> destinataris) {

		var env = NotificacioEnviamentEntity.getBuilderV2(enviament, entitat.isAmbEntregaDeh(), serveiTipus, notificacioGuardada, titular, destinataris, UUID.randomUUID().toString()).build();
		NotificacioEnviamentEntity enviamentSaved = notificacioEnviamentRepository.saveAndFlush(env);
	...
	public NotificacioEnviamentEntity updateEnviament(EntitatEntity entitat, NotificacioEntity notificacioEntity, Enviament enviament,
													  ServeiTipusEnumDto serveiTipus, PersonaEntity titular) {

		var enviamentEntity = notificacioEnviamentRepository.findById(enviament.getId()).orElseThrow();
	...
	@Audita(entityType = TipusEntitat.ENVIAMENT, operationType = TipusOperacio.UPDATE)
	public NotificacioEnviamentEntity updateEnviamentEnviat(NotificacioEntity notificacio, NotificacioEventEntity event, String identificadorResultat,
	...
	@Audita(entityType = TipusEntitat.ENVIAMENT, operationType = TipusOperacio.UPDATE)
	public NotificacioEnviamentEntity updateEnviamentEmailFinalitzat(NotificacioEnviamentEntity enviament) {
	...
	@Audita(entityType = TipusEntitat.ENVIAMENT, operationType = TipusOperacio.UPDATE)
	public NotificacioEnviamentEntity updateRegistreEnviament(NotificacioEntity notificacioEntity, NotificacioEnviamentEntity enviament, String registreNum,
															  Date registreData, NotificacioRegistreEstatEnumDto registreEstat, boolean totsAdministracio,
