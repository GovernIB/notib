...
@Slf4j
@AllArgsConstructor
public class EnviarEntregaPostalActionExecutor implements BaseMutableResourceService.ActionExecutor<NotificacioResourceEntity, Serializable, RespostaActionExecutor> {
	...
	public RespostaActionExecutor exec(String code, NotificacioResourceEntity entity, Serializable params) throws ActionExecutionException {
		...
		try {
			var ok = notificacioService.enviarEntregaPostal(entity.getReferencia(), false);
			return RespostaActionExecutor.builder().ok(ok).build();
		} catch (Exception ex) {
			var msg = "Error inesperat enviant la entrega postal ";
			log.error("[EnviarEntregaPostalActionExecutor] " + msg + "per la notificacio " + entity.getId());
