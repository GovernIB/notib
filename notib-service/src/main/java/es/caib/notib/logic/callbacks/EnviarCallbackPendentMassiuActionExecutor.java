...
@Slf4j
@AllArgsConstructor
public class EnviarCallbackPendentMassiuActionExecutor implements BaseMutableResourceService.ActionExecutor<CallbackResourceEntity, AccioMassivaParams, CallbackResposta> {
	...
	public CallbackResposta exec(String code, CallbackResourceEntity entity, AccioMassivaParams params) throws ActionExecutionException {
		...
		try {
 			return callbackService.enviarCallback(new HashSet<>(params.getIds()));
		} catch (Exception ex) {
			var msg = "Error inesperat enviant els callbacks pendents ";
