...
public class NotificacioMassivaResumReportGenerator implements BaseReadonlyResourceService.ReportGenerator<NotificacioMassivaResourceEntity, Serializable, NotificacioMassivaResource> {
	...
	public DownloadableFile generateFile(String code, List<?> data, ReportFileType fileType, OutputStream out) {
		...
		var nomFitxer = "resum.csv";
		var mediaType = "text/csv";
		var massiva = (NotificacioMassivaResource) data.get(0);
		//TODO S'ESTÀ DESCARERGANT OK PERÒ NO CONTE LES DADES DE LES REMESES
		try {
			var file = notificacioMassivaService.getResumFile(massiva.getEntitat().getId(), massiva.getId());
			if (file.getContingut() == null || file.getContingut().length == 0) {
				log.error("[NotificacioMassivaZipNotificacioReportGenerator] Error generant el resum de la notificacio massiva " + massiva.getId());
			...
			log.error("[NotificacioMassivaZipNotificacioReportGenerator] Error inesperat generant contingut del resum de la notificacio massiva" + massiva.getId(), ex);
			return DownloadableFile.builder().name(nomFitxer).content(new byte[]{}).contentType(mediaType).build();
		}
	}
