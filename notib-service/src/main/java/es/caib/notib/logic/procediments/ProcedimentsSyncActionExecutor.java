...
/**
 * Acció per a actualitzar els proceidments amb la informació de ROLSAC.
 */
@Slf4j
@AllArgsConstructor
public class ProcedimentsSyncActionExecutor implements BaseMutableResourceService.ActionExecutor<ProcedimentResourceEntity, OrganGestorResource.OrganGestorDir3SyncForm, Boolean> {
	...
	public Boolean exec(String code, ProcedimentResourceEntity entity, OrganGestorResource.OrganGestorDir3SyncForm params) throws ActionExecutionException {
		...
		try {
			...
			procedimentService.actualitzaProcediments(dto);
			return true;
		} catch (Exception ex) {
