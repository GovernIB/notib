...
@Slf4j
@AllArgsConstructor
public class ReenviarAmbErrorActionExecutor implements BaseMutableResourceService.ActionExecutor<NotificacioResourceEntity, Serializable, RespostaActionExecutor> {
	...
	public RespostaActionExecutor exec(String code, NotificacioResourceEntity entity, Serializable params) throws ActionExecutionException {
		...
		try {
			var ok = notificacioService.reenviarNotificacioAmbErrors(entity.getId());
			return RespostaActionExecutor.builder().ok(ok).build();
		} catch (Exception ex) {
			var msg = "Error inesperat reenviant enviaments amb error ";
			log.error("[ReenviarAmbErrorActionExecutor] " + msg + "per la notificacio " + entity.getId());
