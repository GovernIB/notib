...
@Slf4j
@AllArgsConstructor
public class PausarCallbackPendentActionExecutor implements BaseMutableResourceService.ActionExecutor<CallbackResourceEntity, Serializable, CallbackResposta> {
	...
	public CallbackResposta exec(String code, CallbackResourceEntity entity, Serializable params) throws ActionExecutionException {
		...
		try {
 			var ok = callbackService.pausarCallback(entity.getId(), true);
			return CallbackResposta.builder().ok(ok).build();
		} catch (Exception ex) {
			var msg = "Error inesperat pausant el callback pendent ";
			log.error("[PausarCallbackPendentActionExecutor] " + msg + "amb id " + entity.getId() + " - " + ex.getMessage());
