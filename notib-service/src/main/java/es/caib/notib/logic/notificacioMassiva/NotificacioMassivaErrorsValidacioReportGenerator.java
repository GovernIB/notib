...
public class NotificacioMassivaErrorsValidacioReportGenerator implements BaseReadonlyResourceService.ReportGenerator<NotificacioMassivaResourceEntity, Serializable, NotificacioMassivaResource> {
	...
	public DownloadableFile generateFile(String code, List<?> data, ReportFileType fileType, OutputStream out) {
		...
		var nomFitxer = "errors_validacio.csv";
		var mediaType = "text/csv";
		var massiva = (NotificacioMassivaResource) data.get(0);
		try {
			var file = notificacioMassivaService.getErrorsValidacioFile(massiva.getEntitat().getId(), massiva.getId());
			if (file.getContingut() == null || file.getContingut().length == 0) {
				log.error("[NotificacioMassivaZipNotificacioReportGenerator] Error generant el CSV d'errors de validació de la notificacio massiva " + massiva.getId());
				return DownloadableFile.builder().name(nomFitxer).content(new byte[]{}).contentType(mediaType).build();
			}
			return DownloadableFile.builder().name(nomFitxer).content(file.getContingut()).contentType(mediaType).build();
		} catch (Exception ex) {
			log.error("[NotificacioMassivaZipNotificacioReportGenerator] Error inesperat generant contingut del CSV d'errors de validacióde la notificacio massiva" + massiva.getId(), ex);
			return DownloadableFile.builder().name(nomFitxer).content(new byte[]{}).contentType(mediaType).build();
		}
	}
