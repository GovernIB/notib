...
@Slf4j
@AllArgsConstructor
public class ActivarCallbackPendentMassiuActionExecutor implements BaseMutableResourceService.ActionExecutor<CallbackResourceEntity, AccioMassivaParams, CallbackResposta> {
	...
	public CallbackResposta exec(String code, CallbackResourceEntity entity, AccioMassivaParams params) throws ActionExecutionException {
		...
		try {
 			return callbackService.pausarCallback(new HashSet<>(params.getIds()), false);
		} catch (Exception ex) {
			var msg = "Error inesperat activant els callbacks pendents ";
			log.error("[ActivarCallbackPendentMassiuActionExecutor] " + msg + "amb id " + entity.getId() + " - " + ex.getMessage());
