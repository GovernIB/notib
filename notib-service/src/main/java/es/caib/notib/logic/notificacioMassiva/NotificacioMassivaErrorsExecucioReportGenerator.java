...
public class NotificacioMassivaErrorsExecucioReportGenerator implements BaseReadonlyResourceService.ReportGenerator<NotificacioMassivaResourceEntity, Serializable, NotificacioMassivaResource> {
	...
	public DownloadableFile generateFile(String code, List<?> data, ReportFileType fileType, OutputStream out) {
		...
		var nomFitxer = "errors_execucio.csv";
		var mediaType = "text/csv";
		var massiva = (NotificacioMassivaResource) data.get(0);
		try {
			var file = notificacioMassivaService.getErrorsExecucioFile(massiva.getEntitat().getId(), massiva.getId());
			if (file.getContingut() == null || file.getContingut().length == 0) {
				log.error("[NotificacioMassivaZipNotificacioReportGenerator] Error generant el CSV d'errors de execucio de la notificacio massiva " + massiva.getId());
			...
			log.error("[NotificacioMassivaZipNotificacioReportGenerator] Error inesperat generant contingut del CSV d'errors de execucio la notificacio massiva" + massiva.getId(), ex);
			return DownloadableFile.builder().name(nomFitxer).content(new byte[]{}).contentType(mediaType).build();
		}
	}
