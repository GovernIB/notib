...
@Slf4j
@AllArgsConstructor
public class ReactivarConsultaSirActionExecutor implements BaseMutableResourceService.ActionExecutor<NotificacioResourceEntity, Serializable, RespostaActionExecutor> {
	...
	public RespostaActionExecutor exec(String code, NotificacioResourceEntity entity, Serializable params) throws ActionExecutionException {
		...
		try {
			var ok = notificacioService.reactivarSir(entity.getId());
			return RespostaActionExecutor.builder().ok(ok).build();
		} catch (Exception ex) {
			var msg = "Error inesperat reactivant la consulta SIR ";
			log.error("[ReactivarConsultaSirActionExecutor] " + msg + "per la notificacio " + entity.getId());
