...
public class ExportarPermisosUsuariReportGenerator implements BaseReadonlyResourceService.ReportGenerator<NotificacioResourceEntity, AccioMassivaParams , AccioMassivaParams > {
...
//							.tipusElementSeleccionat(params.getSeleccioTipus())
			...
			if (fitxer == null) {
				log.error("[ExportarPermisosUsuariReportGenerator] Error generant la exportació a excel dels permisos d'usuari");
			...
			log.error("[ExportarPermisosUsuariReportGenerator] Error inesperat en la exporació a excel dels permisos d'usuari", ex);
			return DownloadableFile.builder().name("error_exportacio.xlsx").content(new byte[]{}).contentType(MediaType.APPLICATION_PDF_VALUE).build();
		}
	}
