...
public class ResourceEntityMappingHelper {
	...
	public <E extends ResourceEntity<R, ?>, R extends Resource<?>> E resourceToEntity(R resource, Serializable pkFromResource, Class<E> entityClass, Map<String, Persistable<?>> referencedEntities) throws ResourceNotCreatedException {
		...
		if (builderMethod == null) {
			try {
				...
				throw new ResourceNotCreatedException(resource.getClass(), "Couldn't create new entity instance (resourceClass=" + entityClass + ")", ex);
			}
		}
		...
		callBuilderMethodForResource(resource, builderInstance, builderReturnType);
		...
		E entity = (E)ReflectionUtils.invokeMethod(ReflectionUtils.findMethod(builderReturnType, "build"), builderInstance);
		...
		if (pkFromResource != null) {
			...
			if (idField != null) {
				...
				ReflectionUtils.setField(idField, entity, pkFromResource);
			}
		}
		return entity;
	}

	public <E extends ResourceEntity<R, ?>, R extends Resource<?>> void updateEntityWithResource(E entity, R resource, Map<String, Persistable<?>> referencedEntities) {
		...
		ReflectionUtils.doWithFields(entity.getClass(), field -> {
			if (Persistable.class.isAssignableFrom(field.getType()) && referencedEntities != null) {
				...
				Method setMethod = ReflectionUtils.findMethod(entity.getClass(), setMethodName, field.getType());
				if (setMethod != null) {
					ReflectionUtils.invokeMethod(setMethod, entity, referencedEntity);
				}
		...
		objectMappingHelper.map(resource, entity, ignoredFieldNames.toArray(new String[0]));
	}

	public <E extends ResourceEntity<R, ?>, R extends Resource<?>> R entityToResource(E entity, Class<R> resourceClass, String... ignoredFields) {
		...
		return objectMappingHelper.newInstanceMap(entity, resourceClass, ignoredFieldNames.toArray(new String[0]));
	}

	private <R> void callBuilderMethodForResource(R resource, Object builderInstance, Class<?> builderReturnType) {
		...
		resourceMethod.ifPresent(method -> ReflectionUtils.invokeMethod(method, builderInstance, resource));
	}
	...
	private void setFileReferenceFieldValue(Field field, Object target) {
		...
		Method setMethod = ReflectionUtils.findMethod(target.getClass(), setMethodName, byte[].class);
		if (setMethod == null) {
			return;
		}
		byte[] fileValue = null;
		if (FileReference.class.isAssignableFrom(field.getType())) {
			...
			if (fileReference != null) {
				fileValue = fileReference.getContent();
			}
		} else {
			fileValue = (byte[])ReflectionUtils.invokeMethod(getMethod, target);
		}
		if (fileValue == null || fileValue.length != 0) {
			ReflectionUtils.invokeMethod(setMethod, target, (Object)fileValue);
		}
	}

	private void addResourceConfigIgnoredFields(Class<?> resourceClass, Set<String> ignoredFieldNames) {
