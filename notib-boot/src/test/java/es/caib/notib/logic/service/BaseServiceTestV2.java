...
//		}
	...
	protected void addConfig(String key, String value) {
		ConfigTypeEntity configType = configRepository.findConfigTypeByValue("TEXT");
		ConfigEntity configEntity = ConfigEntity.builder().key(key).value(value).type(configType).position(0).jbossProperty(false).configurable(false).build();
