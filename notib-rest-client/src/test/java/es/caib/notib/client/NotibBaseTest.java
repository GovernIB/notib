package es.caib.notib.client;

import java.io.FileInputStream;
import java.io.IOException;
import java.io.InputStream;
import java.util.ArrayList;
import java.util.Date;
import java.util.GregorianCalendar;
import java.util.List;
import java.util.Properties;

import javax.xml.datatype.DatatypeConfigurationException;
import javax.xml.datatype.DatatypeFactory;
import javax.xml.datatype.XMLGregorianCalendar;

import es.caib.notib.ws.notificacio.DomiciliConcretTipusEnum;
import es.caib.notib.ws.notificacio.DomiciliNumeracioTipusEnum;
import es.caib.notib.ws.notificacio.DomiciliTipusEnum;
import es.caib.notib.ws.notificacio.NotificaEnviamentTipusEnumDto;
import es.caib.notib.ws.notificacio.NotificacioDestinatari;
import es.caib.notib.ws.notificacio.NotificacioEstatEnumDto;
import es.caib.notib.ws.notificacio.NotificacioSeuEstatEnumDto;
import es.caib.notib.ws.notificacio.Notificacio_Type;
import es.caib.notib.ws.notificacio.ServeiTipusEnum;

public class NotibBaseTest {

	protected Properties properties;
	
	public NotibBaseTest() {
		super();
		this.carregaProperties();
	}

	protected void carregaProperties() {
		// Llegim el fitxer de properties
		properties = new Properties();
		InputStream input = null;

		try {

			input = new FileInputStream("src/test/resources/es/caib/notib/client/test.properties");

			// load a properties file
			properties.load(input);

		} catch (IOException ex) {
			ex.printStackTrace();
		} finally {
			if (input != null) {
				try {
					input.close();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
		}
	}
	
	protected Notificacio_Type createNotificacio() throws DatatypeConfigurationException {
		Notificacio_Type notificacio = new Notificacio_Type();
		
		Long notificacioId = System.currentTimeMillis()%100;
		GregorianCalendar c = new GregorianCalendar();
		c.setTime(new Date(0));
		XMLGregorianCalendar date = DatatypeFactory.newInstance().newXMLGregorianCalendar(c);
		
		List<NotificacioDestinatari> destinataris = new ArrayList<>();
		for(int i = 1; i < 3; i++) {
			NotificacioDestinatari destinatari = new NotificacioDestinatari();
			
					destinatari.setReferencia("referencia_" + notificacioId + "_" + i);
					destinatari.setTitularNom("titularNom_" + notificacioId + "_" + i);
					destinatari.setTitularLlinatges("titularLlinatges_" + notificacioId + "_" + i);
					destinatari.setTitularNif("tNif" + notificacioId + "_" + i);
					destinatari.setTitularTelefon("tTelefon_" + notificacioId + "_" + i);
					destinatari.setTitularEmail("titularEmail_" + notificacioId + "_" + i);
					destinatari.setDestinatariNom("destinatariNom_" + notificacioId + "_" + i);
					destinatari.setDestinatariLlinatges("destinatariLlinatges_" + notificacioId + "_" + i);
					destinatari.setDestinatariNif("dNif" + notificacioId + "_" + i);
					destinatari.setDestinatariTelefon("dTelefon_" + notificacioId + "_" + i);
					destinatari.setDestinatariEmail("destinatariEmail_" + notificacioId + "_" + i);
					destinatari.setDomiciliTipus(DomiciliTipusEnum.CONCRET);
					destinatari.setDomiciliConcretTipus(DomiciliConcretTipusEnum.NACIONAL);
					destinatari.setDomiciliViaTipus("domVT");
					destinatari.setDomiciliViaNom("domiciliViaNom_" + notificacioId + "_" + i);
					destinatari.setDomiciliNumeracioTipus(DomiciliNumeracioTipusEnum.PUNT_KILOMETRIC);
					destinatari.setDomiciliNumeracioNumero("dmNN_" + notificacioId + "_" + i);
					destinatari.setDomiciliNumeracioPuntKm("dmPK_" + notificacioId + "_" + i);
					destinatari.setDomiciliApartatCorreus("dAC_" + notificacioId + "_" + i);
					destinatari.setDomiciliBloc("domiciliBloc_" + notificacioId + "_" + i);
					destinatari.setDomiciliPortal("domiciliPortal_" + notificacioId + "_" + i);
					destinatari.setDomiciliEscala("domiciliEscala_" + notificacioId + "_" + i);
					destinatari.setDomiciliPlanta("domiciliPlanta_" + notificacioId + "_" + i);
					destinatari.setDomiciliPorta("domiciliPorta_" + notificacioId + "_" + i);
					destinatari.setDomiciliComplement("domiciliComplement_" + notificacioId + "_" + i);
					destinatari.setDomiciliPoblacio("domiciliPoblacio_" + notificacioId + "_" + i);
					destinatari.setNotificaIdentificador(notificacioId + "_" + i);
					destinatari.setDomiciliMunicipiNom("domiciliMunicipiNom_" + notificacioId + "_" + i);
					destinatari.setDomiciliCodiPostal("dCP_" + notificacioId + "_" + i);
					destinatari.setDomiciliProvinciaCodi("PC");
					destinatari.setDomiciliProvinciaNom("domiciliProvinciaNom_" + notificacioId + "_" + i);
					destinatari.setDomiciliPaisCodiIso("PCI");
					destinatari.setDomiciliPaisNom("domiciliPaisNom_" + notificacioId + "_" + i);
					destinatari.setDomiciliLinea1("domiciliLinea1_" + notificacioId + "_" + i);
					destinatari.setDomiciliLinea2("domiciliLinea2_" + notificacioId + "_" + i);
					destinatari.setDomiciliCie(new Integer(8));
					destinatari.setDehObligat(true);
					destinatari.setDehNif("dhN_" + notificacioId + "_" + i);
					destinatari.setDehProcedimentCodi("dehPC");
					destinatari.setServeiTipus(ServeiTipusEnum.URGENT);
					destinatari.setRetardPostal(5);
					destinatari.setCaducitat(date);
					destinatari.setNotificaIdentificador("notificaIdentificador_" + notificacioId + "_" + i);
					destinatari.setSeuRegistreNumero("seuRegistreNumero_" + notificacioId + "_" + i);
					destinatari.setSeuRegistreData(date);
					destinatari.setSeuEstat(NotificacioSeuEstatEnumDto.ENVIADA);
			 destinataris.add(destinatari);
					
		}
		
		String arxiu64 = "";
		
		notificacio.setCifEntitat("12345678Z");
		notificacio.setEnviamentTipus(NotificaEnviamentTipusEnumDto.COMUNICACIO);
		notificacio.setEnviamentDataProgramada(date);
		notificacio.setConcepte("concepte_" + notificacioId);
		notificacio.setPagadorCorreusCodiDir3("pccd3_" + notificacioId);
		notificacio.setPagadorCorreusContracteNum("pccNum_" + notificacioId);
		notificacio.setPagadorCorreusCodiClientFacturacio("ccFacturacio_" + notificacioId);
		notificacio.setPagadorCorreusDataVigencia(date);
		notificacio.setPagadorCieCodiDir3("pcDir3_" + notificacioId);
		notificacio.setPagadorCieDataVigencia(date);
		notificacio.setProcedimentCodiSia("Sia_" + notificacioId);
		notificacio.setProcedimentDescripcioSia("procedimentDescripcioSia_" + notificacioId);
		notificacio.setDocumentArxiuNom("documentArxiuNom_" + notificacioId + ".pdf");
		notificacio.setDocumentContingutBase64(arxiu64);
		notificacio.setDocumentSha1("dcSha1_" + notificacioId);
		notificacio.setDocumentNormalitzat(true);
		notificacio.setDocumentGenerarCsv(true);
		notificacio.setSeuExpedientSerieDocumental("seuesd_" + notificacioId);
		notificacio.setSeuExpedientUnitatOrganitzativa("seuEUO_" + notificacioId);
		notificacio.setSeuExpedientIdentificadorEni("seuExpedientIdentificadorEni_" + notificacioId);
		notificacio.setSeuExpedientTitol("seuExpedientTitol_" + notificacioId);
		notificacio.setSeuRegistreOficina("seuRegistreOficina_" + notificacioId);
		notificacio.setSeuRegistreLlibre("seuRegistreLlibre_" + notificacioId);
		notificacio.setSeuIdioma("seuIdioma_" + notificacioId);
		notificacio.setSeuAvisTitol("seuAvisTitol_" + notificacioId);
		notificacio.setSeuAvisText("seuAvisText_" + notificacioId);
		notificacio.setSeuAvisTextMobil("seuAvisTextMobil_" + notificacioId);
		notificacio.setSeuOficiTitol("seuOficiTitol_" + notificacioId);
		notificacio.setSeuOficiText("seuOficiText_" + notificacioId);
		notificacio.setEstat(NotificacioEstatEnumDto.PROCESSADA);
		notificacio.setDestinataris(destinataris);
		
		return notificacio;
	}
}
