...
JBOSS_PROPS_FILE=/home/jboss/apps/notib/jboss.properties
JBOSS_SYSTEM_PROPS_FILE=/home/jboss/apps/notib/jboss_system.properties
...
#	sed '/<\/socket-binding-group>/i <socket-binding name="proxy-https" port="443"\/>' -i $CONFIG_STANDALONE_FILE
	sed '/<\/socket-binding-group>/i <outbound-socket-binding name="smtp-notib" source-port="0" fixed-source-port="false"><remote-destination host="${env.JBOSS_MAIL_HOST}" port="${env.JBOSS_MAIL_PORT}"\/><\/outbound-socket-binding>' -i $CONFIG_STANDALONE_FILE
