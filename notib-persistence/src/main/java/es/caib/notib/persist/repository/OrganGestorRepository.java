...
public interface OrganGestorRepository extends JpaRepository<OrganGestorEntity, Long> {
	...
	@Query("select count(og) from OrganGestorEntity og " +
			" where " +
			"     (og.entitat = :entitat) " +
	...
	@Query("from OrganGestorEntity og where (og.entitat = :entitat) and og.id in (:ids)")
	...
	@Query(	"select distinct og from ProcSerEntity p left outer join p.organGestor og where p.id in (:procedimentIds)")
	List<OrganGestorEntity> findByProcedimentIds(@Param("procedimentIds") List<Long> procedimentIds);

	@Query(	"select distinct og from OrganGestorEntity og where og.codi in (:organCodis) and :estat = og.estat")
	List<OrganGestorEntity> findByEstatAndCodiIn(@Param("organCodis") List<String> organCodis, @Param("estat") OrganGestorEstatEnum estat);
