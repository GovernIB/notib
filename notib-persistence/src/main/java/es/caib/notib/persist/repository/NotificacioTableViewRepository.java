...
public interface NotificacioTableViewRepository extends JpaRepository<NotificacioTableEntity, Long> {
	...
	@Query("select ntf.id from NotificacioTableEntity ntf " +
            ...
            "and (:#{#filtre.nomesLesMeves} = false or ntf.createdBy.codi = :#{#filtre.usuariCodi}) " +
            "AND (:#{#filtre.nomesFiReintents} = false OR ntf.isLastEventFiReintents = true)" +
            " and (" +
            // PERMISOS
            ...
            "and (:#{#filtre.dataIniciNull} = true or ntf.createdDate >= :#{#filtre.dataInici}) " +
            "and (:#{#filtre.dataFiNull} = true or ntf.createdDate <= :#{#filtre.dataFi}) "+
            "and (:#{#filtre.notMassivaIdNull} = true or (ntf.notificacioMassiva.id IS NOT NULL AND ntf.notificacioMassiva.id = :#{#filtre.notMassivaId}))" +
	...
	@Modifying
	@Query(value = "UPDATE NOT_NOTIFICACIO_TABLE SET LASTMODIFIEDBY_CODI = :codiNou WHERE LASTMODIFIEDBY_CODI = :codiAntic", nativeQuery = true)
