...
public class NotificacioWSTest extends BaseServiceTest {
	...
	public void setUp() {
		...
		boolean enviamentPostal = false;	// l'enviament postal és opcional, per tant el domicili és opcional	
		for (int i = destinatarisCodi; i < destinatarisCodi + 5; i++) {
			destinataris.add(new NotificacioDestinatari(
					...
					enviamentPostal ? DomiciliTipusEnum.CONCRET : null,
		...
		//TODO: esbrinar si la codificació sha1 s'ha de guardar en base 64 o no, mirar la documentacó de notifica.
...
//				"pcied3_" + notificacioId);
		...
		notificacio.setDocumentContingutBase64(
				arxiu64);
		notificacio.setDocumentSha1(sha1); //"documentSha1_" + notificacioId);
